...
function save_item(product_id, item_type){
	...
	if($("#item_title").val().replace(/\s+/g, '').length) {
		...
		var content = htmlspecialchars(FCKeditorAPI.GetInstance('post_content').GetXHTML());
		var charecters = "";
		if($('#item_charecters').val() != undefined)
		charecters = htmlspecialchars(FCKeditorAPI.GetInstance('item_charecters').GetXHTML());
...
function add_ann_item(){
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('#items_sortable')
                    .append(
                    '<li id="item_'+ item_id +'" class="ui-state-default" style="margin-bottom:3px;">' +
                        '<div style="float: left; width: 280px;">' +
                            '<div style="float: left; margin: 3px 0 0 5px; width: 250px;">'+ item_title +'</div>' +
                            '<div style="float: right;">' +
                                '<img title="удалить" src="'+ base_url +'images/icons/cancel.png" onclick="delete_ann_item(\''+ item_id +'\');" style="bottom: 3px; cursor: pointer; width: 17px; height: 17px; position: relative; "/>' +
                            '</div>' +
                        '</div>' +
                    '</li>' );
        },

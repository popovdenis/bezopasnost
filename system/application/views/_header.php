...
<div class="header">
    <div>
        <div id="search_menuitems" class="menu_item">
            <form id="quickSearch" action="<?=base_url()?>search" method="post" enctype="application/x-www-form-urlencoded">
                <input id="main_keywords" name="main_keywords" type="text" onkeyup="javascript:main_quick_search();" value="Быстрый поиск">
                <img src="<?=base_url()?>images/search.png" style="cursor:pointer;position: relative; top: 7px; width: 30px; height: 30px;" onclick="javascript: jQuery('#quickSearch').submit();">
            </form>
		...
		<?php
			...
			foreach ($main_cats as $cat) {
				...
				if($key && $key == $_link) $class = 'menu_item_selected';
				
				$link_str .= '<div id="'.$key.'_menuitems" class="'.$class.'">
                  <a href="'.base_url().$key.'">'.$cat->category_title.'</a>
                ';
				if(!empty($cat->subcat)) {
					...
					foreach ($cat->subcat as $index=>$cat2) {
						$style = '';
						if($index == $count) $style = 'style="border:none;"';
                        $slug = $key.'/category/';

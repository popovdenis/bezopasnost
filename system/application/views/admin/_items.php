...
<div id="products_block" style="border:1px solid #CCCCCC;border-top:none;">
	...
	<div id="new_item">
		<div class="left">
			<div class="left add_item"><span onclick="javascript:get_new_page('<?=$item_type?>');">Новая статья</span></div>
			<div style="float:left;"><img id="add_item_img" style="position:relative;top:10px;display:none;" src="<?=base_url()?>images/ajax-loader.gif"/></div>
		</div>
		<div class="right">
			<div class="delete_all_btn" onclick="javascript:if(confirm('Выбранные статьи удалятся вместе с прикрепленным к ним материалом. Вы уверены, что хотите удалить выбранные статьи?'))delete_items_checked();"><span>Удалить отмеченные</span></div>
		</div>
	</div>
	<?php if(isset($categories) && !empty($categories)) { $current = array_pop($categories); ?>
	<div class="left" style="margin:0 0 10px 10px;">
		<span id="filter_items">Фильтр:</span>		
		<select id="item_categories" onchange="javascript:filter_items_category('<?=$item_type?>');">
			<option id="<?=$current->category_id?>" selected><?=$current->category_title?></option>

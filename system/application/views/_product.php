...
<script type="text/javascript">
	...
	<div class="content">
		...
		<div class="infocontent">
			<?=$header_links?>
            <div style="float:left;margin-top:5px;text-align:right;width:100%;"><a href="<?=base_url().'products/subcat/'.$current_catid.'/about/'.$product->item_id.'/print'?>" target="_blank">Версия для печати и PDA</a></div>
            <div>
				<div class="product_title"><?=$product->item_title?></div>
				<?php $next_link = "#"; if($next) $next_link = base_url().'products/subcat/'.$current_catid.'/about/'.$next; ?>
				<?php $prev_link = "#"; if($prev) $prev_link = base_url().'products/subcat/'.$current_catid.'/about/'.$prev; ?>
				<a href="<?=$next_link?>"><div class="next_product">>></div></a>
				<a href="<?=$prev_link?>"><div class="next_product"><<</div></a>
			</div>
			...
			<div class="product_info">
				...
				if(isset($product->attach) && !empty($product->attach)) { ?>
					<a href="<?=base_url().$product->attach->attach_path?>" class="highslide" onclick="return hs.expand(this)">
						<img src="<?=base_url().$product->attach->attach_preview_path?>" title="Click to enlarge" />
					</a>
				<?php } ?>
				</div>
				...
				<?php if(!empty($product->item_price)) { ?>
                    <div style="float: left; font-size: 17px; margin-top: 10px;color:#CC6600;">
                    	<div class="floatL" style="bottom:7px;position:relative;"><img src="<?=base_url()?>images/icons/money.png" /></div>                    	                   
                        <div class="floatL">
                        	<span>Цена:</span>
                ...
                <?php if( $product->item_title != 'АКЦИЯ' ) { ?>
                    <div style="float:right;margin-top:10px;text-align:right;">
                    	<a onclick="javascript:open_compare('<?=$current_catid?>', '<?=$product->item_id?>');return false;" href="#">Сравнить</a>
                    </div>
                <?php } ?>
                    </div>
                </div>			
            ...
            <div class="floatR">
                <?php $next_link = "javascript:;"; if($next) $next_link = base_url().'products/subcat/'.$current_catid.'/about/'.$next; ?>
                <?php $prev_link = "javascript:;"; if($prev) $prev_link = base_url().'products/subcat/'.$current_catid.'/about/'.$prev; ?>
                <a href="<?=$next_link?>"><div class="next_product">>></div></a>
                <a href="<?=$prev_link?>"><div class="next_product"><<</div></a>
            </div>
                                ...
                                $gal_str .= '<li id="' . $attach->attach_id . '">
                                    <div class="product_preview">
                                            ...
                                            <div style="position:relative;bottom:0px;">
                                                ...
                                                <a style="font-size:14px;" href="'.base_url().'products/download/'.$product->item_id.'/category/'.$current_catid.'/attach_id/'. $attach->attach_id .'">Скачать файл</a>
                                            </div>
                                        </div></div></div></div></div></div></div></div>
                                    </div>
                                </li>';

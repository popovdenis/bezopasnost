...
foreach ($items as $item) {
    $item_preview = "&nbsp;";
    if (mb_strlen($item->item_preview) > 83) {
        $item_preview = mb_substr($item->item_preview, 0, 80) . "..";
    } else {
        $item_preview = $item->item_preview;
    }
    $style = 'height:105px;';
    if (isset($item->attach->attach_preview_path) && ! empty($item->attach->attach_preview_path)) {
        $style .= 'background:url(\'' . base_url() . $item->attach->attach_preview_path . '\');';
    }

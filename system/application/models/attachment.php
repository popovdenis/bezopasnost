<?php
	...
	class Attachment extends Model {
		...
		function get_attach($attach_id=null){
			...
			if($attach_id) $query .= " and attach_id=".clean($attach_id);
			
			if(!$query = $this->db->query($query)) return false;						
			return $query->result();
		}
		...
		function update_attachment($attach_id, $info) {			
			try {
				...
				/*$duplicate = null;			
				if(isset($info['attach_title'])) $duplicate = $this->get_attach_by_title($info['attach_title']);
				else $duplicate = $this->get_attach($attach_id);				
				...
				if($duplicate) {
					...
					$document = $query->row();
					$document->attach_path = str_replace("\\", "/", $document->attach_path);
					if(is_file($document->attach_path)) unlink($document->attach_path);
		...
		function delete_attach_item($attach_id, $item_id){
			try {
				$query = "select * from attachment_item where attach_id=".clean($attach_id)." and item_id=".clean($item_id);

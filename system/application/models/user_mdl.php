...
class User_mdl extends Model
{
    ...
    function pre_activate($login, $email, $language, $activation_code)
    {
        ...
        $query           = 'insert into users (login, email, activation_code, registration_date, registration_ip, group_id, language)
              values (' . clean($login) . ', ' . clean($email) . ', ' . clean(
                $activation_code
            ) . ', now(), ' . clean($registration_ip) . ', ' . clean($group_id) . ', ' . clean($language) . ')';
        $query           = $this->db->query($query);
    ...
    {
        $query = $this->db->query(
            'select login, email from users where user_id = ' . clean(
                $activation_user_id
            ) . ' and activation_code=' . clean($activation_code)
        );
        if ($query->num_rows() > 0) {
            $rs = $query->result();
    ...
    function get_users($user_id = null, $extra = '', $order_by = 'user_login', $limit = null)
    {
        ...
        if ($user_id) {
            $query .= ' where user_id=' . clean($user_id);
        }
        if ($user_id && $extra) {
            $query .= " and ";
        }
    ...
    function activate($activation_user_id, $activation_code)
    {
        ...
        $query        = 'update users u, khacl_aros g set
                ...
                and u.user_id = ' . clean($activation_user_id) . ' and u.activation_code=' . clean(
                $activation_code
            );
        $query        = $this->db->query($query);
    ...
    {
        if (empty($data) || !is_array($data) || empty($user_id)) {
            return false;
        }
        $user_fields = array(
            'user_login',

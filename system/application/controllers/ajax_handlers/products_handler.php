<?php
	...
	class Products_handler extends Controller
	{
		...
		function ajax_actions(){
			...
			switch ($action) {
				...
				case "filter_products":
					...
					if($filter_name == "za") $order_by = " order by i.item_content desc ";
					elseif($filter_name == "az") $order_by = " order by i.item_content asc ";

					if($filter_size != 'all') $this->per_page = $filter_size;
					else $this->per_page = 0;
				...
				case "compare":
					...
					if(empty($compare)) $compare = array();
		...
		function get_items_block($cur_page, $category_id, $item_mode = true, $extras = '', $order_by = 'order by i.item_added desc'){
			$this->load->model('items_mdl','items');
			$this->load->model('attachment');

			$page = $cur_page;
			$items = $this->items->get_item(null, $this->slug, $item_mode, $category_id, $this->per_page, $page, true, $extras, $order_by);

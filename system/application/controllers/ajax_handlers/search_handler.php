...
//			log_message('debug', 'keywords:'.$keywords);
			if(empty($keywords)) {
			...
			switch ($action) {
				...
				case "main_quick_search":
					...
					if(!empty($keywords)) {
						...
						$info_cat_ids = array();
						if($info_cat) {
							if(is_array($info_cat)) $info_cat = $info_cat[0];
						...
						if($items) {
							...
							$style= '';
							if($index == $count) {
							    if($items_all > 10) $style= 'style="border: medium none ;"';
							}
							if($item->category_title == 'Бренды' || in_array($item->category_id, $info_cat_ids))						
								$items_str .= '<li '.$style.'><a href="'.base_url().$item->item_type.'/category/'.$item->category_id.'">'.$item->item_title.'</a></li>';
							else
								$items_str .= '<li '.$style.'><a href="'.base_url().$item->item_type.'/subcat/'.$item->category_id.'/about/'.$item->item_id.'">'.$item->item_title.'</a></li>';								
						    }
						}
						if($items_all > 10) $items_str.= '<li style="border: medium none ;"><a href="'.base_url().'search#'.urlencode($keywords).'" onclick="javascript:go_to_search();return false;">Подробнее</a></li>';
					}
					$data = $items_str;
		...
		function _get_items_block($page=1, $keywords = "", $category_id=null, $type='main'){
			...
			if($category_products && is_array($category_products)) $category_products = $category_products[0];
			...
			if(!empty($items)){
				...
				$search_category .= '</li>';
				foreach($items as $index=>$item) {
					if(!isset($item->category_title)) continue;
					$class = '';					
			...
			$info = array('template' => $search_result, 'count' => $items_all, 'paginate_args' => paginate_ajax($paginate_args), 'main_category' => $search_category, 'page' => $page, 'per_page' => $this->per_page);
			return $info;
		}
		
		/**
		 * Destructor of Search_handler 
		 *
		 * @access  public
		 */
		function _Search_handler() {}		
	}
?>
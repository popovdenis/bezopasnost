...
class Users extends Controller {
	...
	function authorize($login, $password) {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if($currency_uah) {
				if(is_array($currency_uah)) $currency_uah = $currency_uah[0];
				$currency_rate = $this->currency->get_currency_rate($currency_uah->currency_id);
				if($currency_rate && is_array($currency_rate)) $currency_rate = $currency_rate[0];
			...
			if ($auth_user) {
				...
				$this->db_session->set_userdata('user_last_login', ($auth_user->last_login_date) ? $auth_user->last_login_date : null);
	...
	function forgot_password($username) {
		...
		if($user) {
			$user_email = null;
			if(!empty($user->user_email1)) {
				$user_email = $user->user_email1;
			} else if (!empty($user->user_email2)) {
				$user_email = $user->user_email2;
			} else {
			...
			$new_password = chr(rand(ord('a'),ord('z'))).rand(1,9).rand(ord('a'),ord('z')).rand(1,9).rand(1,9).rand(ord('a'),ord('z'));
			$new_password_rand = md5($new_password . self::PRIVATE_KEY);

			$data_user = array(
				'password' => $new_password_rand
			);
			$this->user->update_user($user->user_id, $data_user);

			$this->load->helper('email');
			$email = $user_email;
			$subject = "ClickFuel Password Retrieval";
			$message = "Your password for your ClickFuel Fuel Station account is ".$new_password;
			$message .= "<br />Please try to log in again<br />".base_url()." (<a href='".base_url()."'>".base_url()."</a>)";

			$email_cfg = array('mailtype'=>'html','wordwrap'=>FALSE);

			if(send_email($email, $subject, $message, $email_cfg)){
				$error_msg = '<span style="font-size:12px">We have sent your password to the email address for this account</span>';
			}
		} else {
			$error_msg = '<span style="font-size:12px;color:#FF0000;">Error - we did not find that username</span>';
		}
		return $error_msg;
	}

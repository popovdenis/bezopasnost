        ...
        function category(){
            ...
            if($main_cats && !empty($main_cats)) {
                foreach ($main_cats as $category) {
                    $attachments = $this->category->get_category_attachment($category->category_id, null, 'category_title');
                    if($attachments && is_array($attachments)) $attachments = $attachments[0];
            ...
            if(!empty($category_id)) {
                ...
                if(!empty($item_id)) {
                    ...
                    if($current_cat && is_array($current_cat)) $current_cat = $current_cat[0];
...
//			$data['gallery'] = $this->gallery->get_gallery($item->item_id);
		...
		function get_items_block($category_id = null){
			...
			$items = $this->items->get_item(null, $this->slug, true, $category_id, $this->per_page, $this->cur_page, true);		
			...
			if($category && is_array($category)) $category = $category[0];
			...
			if($category->category_title == 'Прайсы')
				$items_str = $this->load->view('all_items_price', $data, true);
			else
		...
		function get_head_links($category_id, $product_id = null){
			$main = $this->category->get_category(null, null, $this->category_title);
			if($main && is_array($main)) $main = $main[0];
			...
			foreach ($categories as $category){
				if($category->category_title == 'root') continue;
				
				if($category->category_id == $main->category_id) $link = base_url().$this->slug.'/';
				elseif($category->category_parent == $main->category_id) $link = base_url().$this->slug.'/category/'.$category->category_id;
				else $link = base_url().$this->slug.'/subcat/'.$category->category_id;
				...
				elseif(!$product_id)
					...
					if($product && is_array($product)) $product = $product[0];
		...
		function download(){
			...
			if($item_id) {
				...
				if(!empty($attaches) && is_array($attaches)) $attaches = $attaches[0];
				
				$data = file_get_contents(base_url().$attaches->attach_path);
				force_download($attaches->attach_path, $data);
			}
			if($category_id) redirect('information/category/'.$category_id);

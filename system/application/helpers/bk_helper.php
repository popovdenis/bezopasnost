<?php
	...
	function image_watermark($main_img, $watermark_img, $attach_ext, $padding=3, $opacity=30){
		...
		if(!empty($attach_ext)) $attach_ext = strtolower($attach_ext);
			
		$watermark 	= imagecreatefrompng($watermark_img); // create watermark
		if($attach_ext == ".jpg" || $attach_ext == ".jpeg") $image = imagecreatefromjpeg($main_img); // create main graphic
		elseif($attach_ext == ".png") $image = imagecreatefrompng($main_img); // create main graphic
		elseif($attach_ext == ".gif") $image = imagecreatefromgif($main_img); // create main graphic
